...
cv2.createTrackbar("Focus", camera_setting_window_name, int(cam.get(cv2.CAP_PROP_FOCUS)), 255, on_focus_change)

while True:
    ret, frame = cam.read()
    if not ret:
        raise Exception("Камера не вернула картинку")
    cv2.putText(frame, f"Current focus is: {int(cam.get(cv2.CAP_PROP_FOCUS))}", (10, 30),
...
while True:
    ...
    cv2.putText(frame, f"Frames for calibration: {len(all_charuco_corners)}", (10, 30),
                cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 1)
    ...
    if key == ord("a"):
        ...
        projection_error_calibration, camera_matrix, dist_coeffs, _, _ = cv2.aruco.calibrateCameraCharuco(
            all_charuco_corners, all_charuco_ids, calibration_board, grey.shape, None, None
        )
        cv2.destroyAllWindows()
        break
#####################################################################
...
while True:
    ...
    m_rvecs, m_tvecs, _ = cv2.aruco.estimatePoseSingleMarkers(corners, BOARD_MARKER_LENGTH_M, camera_matrix, dist_coeffs)
    anchor_rvec = None
    anchor_tvec = None
    if m_rvecs is not None and ids is not None:
        for i, marker_id in enumerate(ids.flatten()):
            if marker_id == PLANE_ANCHOR_MARKER_ID:
                ...
                cv2.drawFrameAxes(frame, camera_matrix, dist_coeffs, anchor_rvec, anchor_tvec, BOARD_MARKER_LENGTH_M * 1.5)
                break
    plane_points = []
...
while True:
    ...
    if not ret:
        raise Exception("Камера не вернула картинку")
    bg = frame
    cv2.putText(frame, "Press 'Enter' to continue", (10, 30),
                cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
...
# 6) Детекция объектов через вычитание фона с ROI
#####################################################################
...
# 7) Поиск похожих контуров и вычисление их 3D координат на плоскости
#####################################################################
...
def pixel_to_3d(pixel_x, pixel_y, normal, d, camera_matrix):
    ...
    if abs(denominator) < 1e-6:
        return None
    lambda_val = -(np.dot(normal, C) + d) / denominator
    ...
    if template_contour is not None and plane_normal is not None and plane_d is not None:
        ...
        for contour in contours:
            ...
            if M["m00"] != 0:
                cX = int(M["m10"] / M["m00"])
                cY = int(M["m01"] / M["m00"])
                if len(contour_buffer) > 0:
                    prev_cX, prev_cY = contour_buffer[-1]
                    if abs(cX - prev_cX) > 10 or abs(cY - prev_cY) > 10:
                        continue
                contour_buffer.append((cX, cY))
                if len(contour_buffer) > 5:
